...
static int check_authn(request_rec *r, const char *username, const char *password){
	...
	do {
		...
		authn_result = authn_dbd_password(r, username, password);
                                       ...
                                       const char *password)
{
    ...
    for (rv = apr_dbd_get_row(dbd->driver, r->pool, res, &row, -1);
		...
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, APLOGNO(55224)
							"auth_jwt authn: Inside 1st for loop...%s",rv);
        if (rv != 0) {
        ...
        if (dbd_password == NULL) {
			ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, APLOGNO(55224)
							"auth_jwt authn: dbd_password...%s",dbd_password);
		...
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, APLOGNO(55224)
							"auth_jwt authn: Inside 2nd for loop...%s",name);
